/**
 * Re-run the 3 hard topics that failed in v3-hard2 — JPMorgan, Riemann,
 * Country — now with BOTH the two-pass hook planner AND the visual
 * metaphor brainstorm step active.
 *
 * Direct comparison:
 *   v3-hard (single pass):       JPM 6, Riemann 4, Country 4
 *   v3-hard2 (two-pass hook):    JPM 7, Riemann 4, Country 2
 *   v3-hard3 (hook + metaphor):  TBD — should crack the abstract image problem
 */
...
const BASE = '/opt/videoforge/output/v3-hard3';
  ...
  for (const t of TOPICS) {
    ...
    try {
      ...
      results.push({
        ...
        metaphor_candidates: (metaphor?.candidates || []).map(c => c.metaphor),
...
console.log(`\n${'═'.repeat(70)}\n  HOOK + METAPHOR BRAINSTORM SUMMARY\n${'═'.repeat(70)}`);
const baseUrl = "http://178.156.209.219:3001/output/v3-hard3";
for (const r of results) {
  ...
  if (r.hook_candidates.length) console.log(`    hook candidates: ${r.hook_candidates.map(c => `"${c}"`).join(', ')}`);
  if (r.metaphor) console.log(`    metaphor: ${r.metaphor}`);
  if (r.metaphor_candidates?.length) {
    console.log(`    metaphor candidates:`);
    for (const m of r.metaphor_candidates) console.log(`      - ${m.substring(0, 100)}`);
  }
