...
function applyPostProcessing(allClips, totalDuration, scriptText, nicheInfo, videoBible = {}) {
  ...
  // Only split STOCK clips in first 15s — never animations (splitting animations
  // just shows the same graphic twice which looks broken).
  // Stock clips > 3s get split with different queries for variety.
  {
    const HOOK_END = 15.0;
    const MAX_HOOK_STOCK = 3.0;
    const hookedClips = [];
    const historyQueries = ["ancient roman ruins","roman empire soldiers","roman colosseum","ancient rome marble","roman legion battle"];
    const hookFallbacks = nicheSafeQueries[nicheInfo?.niche] || nicheSafeQueries.general;
    for (const clip of allClips) {
      const dur = clip.end_time - clip.start_time;
      const inHook = clip.end_time <= HOOK_END;
      const isStock = clip.visual_type === "stock";
      if (inHook && isStock && dur > MAX_HOOK_STOCK) {
        const mid = clip.start_time + dur / 2;
        hookedClips.push({ ...clip, end_time: mid });
        const altQuery = hookFallbacks[Math.floor(Math.random() * hookFallbacks.length)];
        hookedClips.push({ ...clip, start_time: mid, search_query: altQuery });
      } else {
