...
function applyPostProcessing(allClips, totalDuration, scriptText, nicheInfo, videoBible = {}) {
  ...
  // Any clip in the first 15s that's longer than 2.5s gets split into 2 clips.
  // This creates the fast-cut energy the hook needs without touching anything else.
  // We only do this once — no recursion, no touching clips after 15s.
  {
    const HOOK_END = 15.0;
    const MAX_HOOK_CLIP = 2.5;
    const hookedClips = [];
    for (const clip of allClips) {
      if (clip.end_time <= HOOK_END && (clip.end_time - clip.start_time) > MAX_HOOK_CLIP) {
        // Split this clip in half
        const mid = clip.start_time + (clip.end_time - clip.start_time) / 2;
        // First half: keep original type
        hookedClips.push({ ...clip, end_time: mid });
        // Second half: if animation keep it, if stock rotate query
        const secondHalf = { ...clip, start_time: mid };
        if (clip.visual_type === "stock") {
          const fallbackQ = (nicheSafeQueries[nicheInfo?.niche] || nicheSafeQueries.general);
          secondHalf.search_query = fallbackQ[Math.floor(Math.random() * fallbackQ.length)];
        }
        hookedClips.push(secondHalf);
      } else {
