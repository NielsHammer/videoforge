...
const CounterReveal = ({ displayValue, label, clipFrame, fps, th }) => {
  ...
  return (
    <div style={{ position: "absolute", top: "8%", left: 0, right: 0, display: "flex", flexDirection: "column", alignItems: "center", transform: `scale(${scale})`, opacity, zIndex: 10 }}>
      ...
      {label && <div style={{ fontSize: 32, fontWeight: 500, fontFamily: "Arial Black, Arial, sans-serif", color: th.labelColor, opacity: labelOp, textAlign: "center", marginTop: 12, textTransform: "uppercase", letterSpacing: 2 }}>{label}</div>}
    </div>
  );
};

// === STYLE 2: Gauge (circular progress ring) ===
const GaugeReveal = ({ value, displayValue, label, progress, clipFrame, fps, th }) => {
...
const BarsReveal = ({ displayValue, label, progress, clipFrame, fps, th }) => {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity, zIndex: 10 }}>
      <div style={{ fontSize: 100, fontWeight: 900, fontFamily: "Arial Black, Arial, sans-serif", color: "white", letterSpacing: -2, marginBottom: 20 }}>{displayValue}</div>
...
const SpotlightReveal = ({ displayValue, label, clipFrame, fps, th }) => {
...
const TickerReveal = ({ displayValue, label, clipFrame, fps, th }) => {
...
const ImpactReveal = ({ displayValue, label, clipFrame, fps, th }) => {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 10 }}>
      <div style={{ position: "absolute", width: 500, height: 500, borderRadius: "50%", border: "3px solid rgba(255,255,255,0.15)", transform: `scale(${ringScale})`, opacity: ringOp }} />
      <div style={{ fontSize: 150, fontWeight: 900, fontFamily: "Arial Black, Arial, sans-serif", background: th.gradient, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", transform: `scale(${slamScale}) translateX(${shake}px)`, opacity, letterSpacing: -5, lineHeight: 1 }}>
        {displayValue}
      </div>
      {label && <div style={{ fontSize: 34, fontWeight: 700, color: th.labelColor, marginTop: 20, textTransform: "uppercase", letterSpacing: 3, opacity: labelOp, textShadow: `0 0 15px rgba(${th.glowRgb},0.3)` }}>{label}</div>}
    </div>
  );
};
