...
// BeforeAfter: Shows a before/after comparison
// Handles two data formats:
// Format 1 (simple): { before: "$2,000/mo", after: "$12,000/mo", label: "income transformation" }
// Format 2 (detailed): { before: { label: "BEFORE", items: ["item1","item2"] }, after: { label: "AFTER", items: [...] }, title: "" }
export const BeforeAfter = ({ data, clipFrame = 0, theme = "blue_grid" }) => {
  ...
  // Normalize to object format regardless of what was passed
  const isSimple = typeof data.before === "string" || typeof data.after === "string";

  const before = isSimple
    ? { label: "BEFORE", value: String(data.before || ""), items: [] }
    : { label: data.before?.label || "BEFORE", value: "", items: Array.isArray(data.before?.items) ? data.before.items : [] };

  const after = isSimple
    ? { label: "AFTER", value: String(data.after || ""), items: [] }
    : { label: data.after?.label || "AFTER", value: "", items: Array.isArray(data.after?.items) ? data.after.items : [] };
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 60px" }}>
      ...
      <div style={{ display: "flex", width: "100%", gap: 0, alignItems: "stretch" }}>
        {/* Before */}
        <div style={{ flex: 1, opacity: leftOp, transform: `translateX(${leftX}px)`, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: "16px 0 0 16px", padding: "24px 20px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          ...
          {before.value ? (
            <div style={{ fontSize: 48, fontWeight: 900, color: "rgba(255,255,255,0.6)", fontFamily: "Arial Black, Arial, sans-serif", textAlign: "center", textShadow: "0 0 20px rgba(239,68,68,0.4)" }}>
              {before.value}
            </div>
          ) : (
        ...
        {/* After */}
        <div style={{ flex: 1, opacity: rightOp, transform: `translateX(${rightX}px)`, background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: "0 16px 16px 0", padding: "24px 20px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          ...
          {after.value ? (
            <div style={{ fontSize: 48, fontWeight: 900, color: "white", fontFamily: "Arial Black, Arial, sans-serif", textAlign: "center", textShadow: `0 0 30px ${accent}80` }}>
              {after.value}
            </div>
          ) : (
