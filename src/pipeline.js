...
- Feature real, relatable people matching the target audience — NOT models, NOT bodybuilders, NOT glamorous
- Documentary/photojournalism style — candid, natural lighting, eye-level or slightly low angle
- NOT clickbait. NOT stock photo perfection. NOT dramatic studio lighting. NOT overly polished AI aesthetic.
- Show ordinary people as they actually look — average build, real clothing, real environments
- If describing an exercise: show that EXACT exercise being performed with correct form
- If describing a concept: show a real-world scene that represents it, not an abstract visualization
- The image should look like a real photograph taken by a documentary filmmaker, not generated by AI
...
Style: Default,Montserrat,48,&H00FFFFFF,&H0000FFFF,&H00000000,&H80000000,-1,0,0,0,100,100,1,0,1,3,2,2,80,80,120,1
...
`;
  ...
  const lines = phrases.map(phrase => {
    ...
    // Fixed position: bottom center at exact pixel (960, 960) on 1920x1080 canvas
    // This prevents ANY jumping — every phrase renders at the identical spot
    return 'Dialogue: 0,' + toASSTime(phrase.start) + ',' + toASSTime(phrase.end) + ',Default,,0,0,0,,{\\an2\\pos(960,960)}' + kText;
  });
